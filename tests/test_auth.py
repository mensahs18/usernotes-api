...
async def test_register(client):
    response = await client.post(
        ...
        json={
            ...
            "name": {"fname": "testFirstname", "sname": "testSurname"},
        },
    )
...
@pytest.mark.parametrize(
    "dupe_username", ["testuserA", "TESTUSERA", "TeStUsErA", "tesTuseRA "]
)
    ...
    [
        {
            ...
            "name": {"fname": "John", "sname": "Smith"},
        },
        {
            "username": "valid_user",
            "password": ["array", "for", "pw"],
            "name": {"fname": "John", "sname": "Smith"},
        },
        {
            "username": "valid_user",
            "password": "valid_Password1!",
            "name": {"fname": 4.5, "sname": "Smith"},
        },
        {
            "username": "valid_user",
            "password": "valid_Password1!",
            "name": {"fname": "John", "sname": True},
        },
    ],
)
async def test_register_incorrect_data_types(client, login_payload):
    response = await client.post(url="/users/register", json=login_payload)
...
async def test_login(client, username):
    ...
    response = await client.post(
        url="/users/login", data={"username": username, "password": "2!TestPassword"}
    )
...
async def test_login_validation(client, username, password):
    ...
    response = await client.post(
        url="/users/login", data={"username": username, "password": password}
    )
...
@pytest.mark.parametrize(
    "username, password", [("", ""), ("", "1!CorrectPassword"), ("correctuser", "")]
)
...
async def test_login_json(client):
    ...
    response = await client.post(
        url="/users/login", json={"username": "jsonuser", "password": "3!TestPassword"}
    )
...
async def test_malicious_login(client, malicious_input):
    response = await client.post(
        url="/users/login",
        data={"username": malicious_input, "password": malicious_input},
    )
